...
*   **State/NoteDetailViewModel.swift**: 2025-12-28T13:19:42-0500: Loads a single note’s full content via `/api/pull_full_notes` and exposes loading/error state for `NoteDetailView`. Imported by `Views/NoteDetailView.swift`. Imports Foundation, `Models/NoteModels.swift`, `Services/NotesService.swift`.
...
*   **Views/NotesListView.swift**: 2025-12-21T12:06:08-05:00: Read-only note list UI (Apple Notes–style rows) with pull-to-refresh and infinite scroll pagination; navigates to `NoteDetailView` on row tap. Imported by `Views/BrowseFolderDestinationView.swift`. Imports SwiftUI, `State/NotesListViewModel.swift`, `Views/NoteDetailView.swift`, `Theme.swift`.
*   **Views/NoteDetailView.swift**: 2025-12-28T13:19:42-0500: Read-only note detail screen that fetches full content on appear and renders `note_content`. Imported by `Views/NotesListView.swift`. Imports SwiftUI, `State/NoteDetailViewModel.swift`, `Theme.swift`.
